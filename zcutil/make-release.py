...
def main(args=sys.argv[1:]):
    """
    Perform the final Mlgb release process up to the git tag.
    """
...
def update_debian_changelog(release):
    ...
    os.environ['DEBFULLNAME'] = 'Mlgb Company'
    sh_log(
        'debchange',
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "mlgb-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "mlgb-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    ...
    logging.info('mlgb make-release.py debug log: %r', logname)

...
PACKAGE_VERSION=$($SRC_PATH/src/mlgbd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/mlgbd $DEB_BIN
cp $SRC_PATH/src/mlgb-cli $DEB_BIN
cp $SRC_PATH/zcutil/fetch-params.sh $DEB_BIN/mlgb-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/mlgbd.1 $DEB_MAN
cp $SRC_DOC/man/mlgb-cli.1 $DEB_MAN
cp $SRC_DOC/man/mlgb-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/bitcoind.bash-completion $DEB_CMP/mlgbd
cp $SRC_PATH/contrib/bitcoin-cli.bash-completion $DEB_CMP/mlgb-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/mlgbd.1
gzip --best -n $DEB_MAN/mlgb-cli.1
gzip --best -n $DEB_MAN/mlgb-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/mlgbd $DEB_BIN/mlgb-cli

...
Mlgb has been subjected to a formal third-party security review. For security
...
This implementation of Mlgb is not resistant to side-channel attacks. You
should assume (even unprivileged) users who are running on the hardware, or who
are physically near the hardware, that your `mlgbd` process is running on will
be able to:
...
unprivileged) on the hardware your `mlgbd` process runs on until these
...
in [issue #826](https://github.com/mlgb/mlgb/issues/826), and believe that we

...
Nathan Wilcox (4):
      ...
      Add a mlgb-gtest binary to our build with a single tautological test.
      Add coverage support scoped to only the mlgb-gtest run; invoke with make mlgb-cov; make cov is a superset.
...
Sean Bowe (49):
      ...
      Move incremental merkle tree tests to mlgb-gtest.
      ...
      Small nit fixes
      Run `mlgb-gtest` in `make check` and fix performance tests.
      ...
      Introduce new `libmlgb` Mlgb protocol API and crypto constructions surrounding the zkSNARK circuit.
...
Taylor Hornby (7):
      ...
      Fix RPC tests
      Rename bitcoin.conf and bitcoind.pid to mlgb.conf and mlgbd.pid in qa/ and src/

...
Jay Graber (17):
      ...
      Refer to Mlgb wiki in INSTALL
      Rm bitcoin logo
      Rm build-unix.md, to keep single copy of build instructions for Mlgb on github wiki
      Rm Bitcoin-specific documentation
      Add note that document is not updated for Mlgb to translation policy
      Rm doc for disabled REST interface
      Change alpha to beta testnet, add mlgb hidden service
      Improve documentation on connecting to mlgb hidden server
      Improve documentation on connecting to mlgb hidden server
      Update tor.md
...
Kevin Gallagher (24):
      ...
      Adds mlgb/Mlgb.h to LIBZCASH sources
      Adds mlgb/Proof.hpp to LIBZCASH_H
      Add alertkeys.h to libbitcoin_server_a_SOURCES
      Adds files in src/mlgb/circuit to libmlgb_a_SOURCES
      Adds zcbenchmarks.h to libbitcoin_wallet_a_SOURCES
      Adds json_test_vectors.h to mlgb_gtest_SOURCES
      Adds additional licenses to Debian copyright file
      Updates Mlgb Core developers -> Mlgb developers

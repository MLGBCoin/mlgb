...
Jack Grigg (22):
      ...
      Instruct users to run mlgb-fetch-params if network params aren't available
      ...
      Throw an error if mlgb.conf is missing
      Show a friendly message explaining why mlgbd needs a mlgb.conf
      Fix gtest ordering broken by #1949
      Debian package lint
      Generate Debian control file to fix shlibs lint
      Create empty mlgb.conf during performance measurements
      Create empty mlgb.conf during coverage checks
...
Simon Liu (13):
      ...
      Remove UTF-8 BOM efbbbf from mlgb.conf to avoid problems with command line tools
      Closes #1097 so mlgb-cli now displays license info like mlgbd.

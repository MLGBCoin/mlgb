...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or mlgb.conf setting gen)\n"
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "mlgbd compiled without wallet and -mineraddress not set");
#endif
    }
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Mlgb is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Mlgb is downloading blocks...");
...
UniValue getblocksubsidy(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"miner\" : x.xxx           (numeric) The mining reward amount in MGB.\n"
            "  \"founders\" : x.xxx        (numeric) The founders reward amount in MGB.\n"
            "}\n"
            "\nExamples:\n"

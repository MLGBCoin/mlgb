...
static bool DecryptSpendingKey(const CKeyingMaterial& vMasterKey,
                               ...
                               libmlgb::SpendingKey& sk)
{
    CKeyingMaterial vchSecret;
    if(!DecryptSecret(vMasterKey, vchCryptedSecret, address.GetHash(), vchSecret))
        return false;

    if (vchSecret.size() != libmlgb::SerializedSpendingKeySize)
        return false;
...
{
    {
        ...
        {
            const libmlgb::PaymentAddress &address = (*skmi).first;
...
bool CCryptoKeyStore::AddSpendingKey(const libmlgb::SpendingKey &sk)
{
    {
...
bool CCryptoKeyStore::AddCryptedSpendingKey(const libmlgb::PaymentAddress &address,
...
bool CCryptoKeyStore::GetSpendingKey(const libmlgb::PaymentAddress &address, libmlgb::SpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
{
    {
        ...
        {
            const libmlgb::SpendingKey &sk = mSpendingKey.second;
            CSecureDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << sk;
            CKeyingMaterial vchSecret(ss.begin(), ss.end());
            libmlgb::PaymentAddress address = sk.address();
            std::vector<unsigned char> vchCryptedSecret;

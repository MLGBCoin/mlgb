...
bool CWallet::AddZKey(const libmlgb::SpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapZKeyMetadata
...
bool CWallet::AddCryptedSpendingKey(const libmlgb::PaymentAddress &address,
                                    const libmlgb::ViewingKey &vk,
...
bool CWallet::LoadCryptedZKey(const libmlgb::PaymentAddress &addr, const libmlgb::ViewingKey &vk, const std::vector<unsigned char> &vchCryptedSecret)
{
    return CCryptoKeyStore::AddCryptedSpendingKey(addr, vk, vchCryptedSecret);
}

bool CWallet::LoadZKey(const libmlgb::SpendingKey &key)
{
            ...
            for (mapNoteData_t::value_type& item : wtxItem.second.mapNoteData) {
                if (!item.second.nullifier) {
                    ...
                    auto hSig = wtxItem.second.vjoinsplit[i].h_sig(
                        *pmlgbParams, wtxItem.second.joinSplitPubKey);
...
mapNoteData_t CWallet::FindMyNotes(const CTransaction& tx) const
{
    ...
    for (size_t i = 0; i < tx.vjoinsplit.size(); i++) {
        auto hSig = tx.vjoinsplit[i].h_sig(*pmlgbParams, tx.joinSplitPubKey);
...
void CWallet::GetFilteredNotes(std::vector<CNotePlaintextEntry> & outEntries, std::string address, int minDepth, bool ignoreSpent)
{
    ...
    libmlgb::PaymentAddress filterPaymentAddress;
    if (address.length() > 0) {
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapNoteData) {
            ...
            auto hSig = wtx.vjoinsplit[i].h_sig(*pmlgbParams, wtx.joinSplitPubKey);
            try {

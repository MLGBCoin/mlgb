...
CWalletTx GetValidReceive(const libmlgb::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libmlgb::Note GetNote(const libmlgb::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libmlgb::SpendingKey& sk,
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libmlgb::SpendingKey::random();
    auto sk2 = libmlgb::SpendingKey::random();

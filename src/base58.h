...
public:
    bool Set(const libmlgb::PaymentAddress& addr);
    CZCPaymentAddress() {}

    CZCPaymentAddress(const std::string& strAddress) { SetString(strAddress.c_str(), 2); }
    CZCPaymentAddress(const libmlgb::PaymentAddress& addr) { Set(addr); }

    libmlgb::PaymentAddress Get() const;
};

class CZCSpendingKey : public CBase58Data {
public:
    bool Set(const libmlgb::SpendingKey& addr);
    CZCSpendingKey() {}

    CZCSpendingKey(const std::string& strAddress) { SetString(strAddress.c_str(), 2); }
    CZCSpendingKey(const libmlgb::SpendingKey& addr) { Set(addr); }

    libmlgb::SpendingKey Get() const;
};

...
#include "mlgb/Mlgb.h"
#include "mlgb/IncrementalMerkleTree.hpp"
...
double benchmark_create_joinsplit()
{
    ...
    assert(jsdesc.Verify(*pmlgbParams, verifier, pubKeyHash));
    return ret;
}
...
double benchmark_verify_joinsplit(const JSDescription &joinsplit)
{
    ...
    uint256 pubKeyHash;
    auto verifier = libmlgb::ProofVerifier::Strict();
    joinsplit.Verify(*pmlgbParams, verifier, pubKeyHash);
    return timer_stop(tv_start);
}
...
double benchmark_try_decrypt_notes(size_t nAddrs)
{
    ...
    for (int i = 0; i < nAddrs; i++) {
        auto sk = libmlgb::SpendingKey::random();
        wallet.AddSpendingKey(sk);
    }

    auto sk = libmlgb::SpendingKey::random();
    auto tx = GetValidReceive(*pmlgbParams, sk, 10, true);
...
double benchmark_increment_note_witnesses(size_t nTxs)
{
    ...
    for (int i = 0; i < nTxs; i++) {
        auto wtx = GetValidReceive(*pmlgbParams, sk, 10, true);
        auto note = GetNote(*pmlgbParams, sk, wtx, 0, 1);

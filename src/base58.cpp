...
bool CZCPaymentAddress::Set(const libmlgb::PaymentAddress& addr)
{
    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
    ss << addr;
    std::vector<unsigned char> addrSerialized(ss.begin(), ss.end());
    assert(addrSerialized.size() == libmlgb::SerializedPaymentAddressSize);
    SetData(Params().Base58Prefix(CChainParams::ZCPAYMENT_ADDRRESS), &addrSerialized[0], libmlgb::SerializedPaymentAddressSize);
    return true;
}

libmlgb::PaymentAddress CZCPaymentAddress::Get() const
{
    if (vchData.size() != libmlgb::SerializedPaymentAddressSize) {
    ...
    libmlgb::PaymentAddress ret;
    ss >> ret;
    return ret;
}

bool CZCSpendingKey::Set(const libmlgb::SpendingKey& addr)
{
    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
    ss << addr;
    std::vector<unsigned char> addrSerialized(ss.begin(), ss.end());
    assert(addrSerialized.size() == libmlgb::SerializedSpendingKeySize);
    SetData(Params().Base58Prefix(CChainParams::ZCSPENDING_KEY), &addrSerialized[0], libmlgb::SerializedSpendingKeySize);
    return true;
}

libmlgb::SpendingKey CZCSpendingKey::Get() const
{
    if (vchData.size() != libmlgb::SerializedSpendingKeySize) {
        throw std::runtime_error(

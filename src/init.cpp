...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "mlgb.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "mlgbd.pid"));
#endif
...
static void ZC_LoadParams()
{
    ...
    if (!(boost::filesystem::exists(pk_path) && boost::filesystem::exists(vk_path))) {
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the Mlgb network parameters in the following directory:\n"
              "%s\n"
              "Please run 'mlgb-fetch-params' or './zcutil/fetch-params.sh' and then restart."),
                ZC_GetParamsDir()),
    ...
    pmlgbParams = ZCJoinSplit::Unopened();

    LogPrintf("Loading verifying key from %s\n", vk_path.string().c_str());
    gettimeofday(&tv_start, 0);

    pmlgbParams->loadVerifyingKey(vk_path.string());

    gettimeofday(&tv_end, 0);
    elapsed = float(tv_end.tv_sec-tv_start.tv_sec) + (tv_end.tv_usec-tv_start.tv_usec)/float(1000000);
    LogPrintf("Loaded verifying key in %fs seconds.\n", elapsed);

    pmlgbParams->setProvingKeyPath(pk_path.string());
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Mlgb is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Mlgb is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Mlgb is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // Initialize Mlgb circuit parameters
    ZC_LoadParams();
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("Mlgb was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Mlgb with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("Mlgb was not built with wallet support. Set -mineraddress, or rebuild Mlgb with wallet support."));
    }
 #endif // !ENABLE_WALLET

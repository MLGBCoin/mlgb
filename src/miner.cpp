...
#endif // ENABLE_WALLET
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MlgbMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("MlgbMiner: ProcessNewBlock, block not accepted");
...
#endif
{
    LogPrintf("MlgbMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mlgb-miner");
    const CChainParams& chainparams = Params();
    ...
    try {
        while (true) {
            ...
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in MlgbMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in MlgbMiner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            LogPrintf("Running MlgbMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("MlgbMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("MlgbMiner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();

...
#if defined(NDEBUG)
# error "Mlgb cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Mlgb Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state,
                      libmlgb::ProofVerifier& verifier)
{
    ...
    if (!CheckTransactionWithoutProofVerification(tx, state)) {
        ...
        BOOST_FOREACH(const JSDescription &joinsplit, tx.vjoinsplit) {
            if (!joinsplit.Verify(*pmlgbParams, verifier, tx.joinSplitPubKey)) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    auto verifier = libmlgb::ProofVerifier::Strict();
    auto disabledVerifier = libmlgb::ProofVerifier::Disabled();
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // In Mlgb this has been enforced since launch, except that the genesis
    // block didn't include the height in the coinbase (see Mlgb protocol spec
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    auto verifier = libmlgb::ProofVerifier::Disabled();
